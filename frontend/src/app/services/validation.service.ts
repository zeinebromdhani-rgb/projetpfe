...
export class ValidationService {
  ...
  static passwordMatchValidator(passwordKey: string, confirmPasswordKey: string): ValidatorFn {
    return (group: AbstractControl): { [key: string]: any } | null => {
      ...
      // If confirm is empty, don't show mismatch error yet
      if (!confirmPassword) {
      ...
      if (password !== confirmPassword) {
        // attach error to confirmPassword control
        confirmPasswordControl.setErrors({
          ...confirmPasswordControl.errors,
          passwordMismatch: true
        });
      } else {
        // remove only passwordMismatch, keep other possible errors
        if (confirmPasswordControl.errors) {
          const { passwordMismatch, ...rest } = confirmPasswordControl.errors;
