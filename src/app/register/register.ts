...
export class RegisterComponent {
  ...
  onSubmit(): void {
    if (this.registerForm.valid && this.acceptTerms) {
      ...
      if (this.isEmailAlreadyUsed(registerRequest.email)) {
  ...
  private isEmailAlreadyUsed(email: string): boolean {
    const usedEmails = ['admin@monsite.com', 'test@test.com', 'user@example.com'];
  ...
  onNameInput(): void {
    ...
    if (nameControl?.value) {
      let cleanedName = nameControl.value
        .replace(/[^a-zA-ZÀ-ÿ\s\-']/g, '')
        .replace(/\s+/g, ' ')
        .replace(/^[\s\-']+|[\s\-']+$/g, '');
      if (cleanedName !== nameControl.value) {
        nameControl.setValue(cleanedName);
      }
    }
  }
